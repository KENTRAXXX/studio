...
function SignUpFormContent() {
  ...
  const onGoogleSignUp = () => {
    ...
    if (planTier === 'ADMIN') {
        const systemSecret = process.env.NEXT_PUBLIC_ADMIN_GATE_CODE;
        if (systemSecret && form.getValues('adminCode') !== systemSecret) {
            toast({
                variant: 'destructive',
                title: 'Authorization Denied',
                description: 'The provided Executive Access Code is invalid.',
    ...
    const { fullName, email, password, ...restData } = form.getValues();
                    ...
                    <Card className="border-primary/50 bg-card/50 backdrop-blur-md">
                        ...
                        <CardContent>
                            ...
                            <Button 
                              ...
                              className="w-full h-14 text-md border border-primary/50 bg-primary/10 text-primary hover:bg-primary/20 font-bold transition-all shadow-[0_0_15px_rgba(212,175,55,0.15)]"
                            >
