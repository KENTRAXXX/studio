    ...
    return (
        <div className="flex flex-col items-center min-h-screen bg-background p-4 sm:p-6 text-foreground">
            ...
            <Tabs value={interval} onValueChange={(value) => setInterval(value as Interval)} className="w-full max-w-sm mb-8">
                <TabsList className="grid w-full grid-cols-2 bg-muted/50 p-1 border border-primary/10">
                ...
                {plans.map(plan => {
                    ...
                    >
                        ...
                        <CardHeader className="text-center items-center">
                            ...
                            {isDiscounted && (
                                <p className="text-[10px] text-muted-foreground line-through decoration-destructive mt-1">
                                    Was ${priceInfo.price}
                                </p>
                            )}
                        </CardHeader>
            ...
            <p className="text-center text-sm text-muted-foreground mt-6 max-w-md italic">
                All plans include custom domain mapping, SSL provisioning, and unlimited Master Catalog cloning.
            </p>
        </div>
    );
}
